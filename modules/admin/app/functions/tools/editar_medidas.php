...
if ($resultPanelId->num_rows > 0) {
    ...
    // Verifica se o ID da medida foi fornecido na URL
    if (isset($_GET['id'])) {
        $medidaId = $_GET['id'];

        // Query para obter os detalhes da medida com base no panel_id e no ID fornecido
        $sql = "SELECT * FROM medidas WHERE panel_id = $panel_id AND id = $medidaId";
        $result = $conn->query($sql);

        if ($result->num_rows > 0) {
            $medida = $result->fetch_assoc();
            // Agora você pode usar $medida para acessar os detalhes da medida
        } else {
            echo "Medida não encontrada.";
            exit();
        }
    } else {
        echo "ID da medida não fornecido na URL.";
        exit();
    }
} else {
...
<html lang="en">
  ...
  <body>
    ...
    <div class="br-mainpanel">
      <div class="br-pageheader pd-y-15 pd-l-20">
        <nav class="breadcrumb pd-0 mg-0 tx-12">
          ...
          <a class="breadcrumb-item" href="#">Medidas</a>
          <span class="breadcrumb-item active">Editar Medidas</span>
        </nav>
      </div><!-- br-pageheader -->
      <div class="pd-x-20 pd-sm-x-30 pd-t-20 pd-sm-t-30">
        <h4 class="tx-gray-800 mg-b-5">Editar Medida</h4>
        <p class="mg-b-0">Edite a medida selecionada.</p>
      </div>

      <div class="br-pagebody">
        <div class="br-section-wrapper">

        <form action="atualizar_medida.php" method="post" class="mt-4">
    ...
    <div class="form-group">
        <label for="nome_medida">Nome da Medida:</label>
        <input type="text" name="nome_medida" class="form-control" value="<?php echo $medida['nome_medida']; ?>" required>
    </div>
