...
if (isset($_GET['id'])) {
    ...
    if ($resultPanelId->num_rows > 0) {
        ...
        $sqlCheckOwnership = "SELECT id FROM complementos_adicionais WHERE panel_id = $panel_id AND id = $complementoId";
        ...
        if ($resultCheckOwnership->num_rows > 0) {
            ...
            $sqlDelete = "DELETE FROM complementos_adicionais WHERE id = $complementoId";
            ...
            if ($conn->query($sqlDelete) === TRUE) {
                ...
                echo "Erro ao excluir complemento: " . $conn->error;
            }
        } else {
            echo "Você não tem permissão para excluir este complemento.";
        }
    } else {
