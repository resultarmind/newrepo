...
if ($resultPanelId->num_rows > 0) {
    ...
    $sqlProdutos = "SELECT * FROM complementos WHERE panel_id = '$panel_id'";
    ...
    <div class="br-mainpanel">
      <div class="br-pageheader pd-y-15 pd-l-20">
        <nav class="breadcrumb pd-0 mg-0 tx-12">
          ...
          <span class="breadcrumb-item active">Visualizar Complementos</span>
        </nav>
      </div><!-- br-pageheader -->
      <div class="pd-x-20 pd-sm-x-30 pd-t-20 pd-sm-t-30">
        <h4 class="tx-gray-800 mg-b-5">Visualizar Complementos</h4>
        <p class="mg-b-0">Visualize ou edite as complementos existentes.</p>
      </div>
...
while ($produto = $resultProdutos->fetch_assoc()) {
    ...
    // Link para editar
    echo '<a href="functions/tools/editar_complemento.php?id=' . $produto["id"] . '" class="btn btn-primary btn-sm">Editar</a>';
...
function confirmDelete(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            window.location.href = 'functions/tools/excluir_complemento.php?id=' + id;
        }
    });
}
</script>
